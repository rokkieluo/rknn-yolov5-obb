...
void get_results(int width = 1280, int height = 720, int fps = 30, bool record = false)
{
    // int64_t frame_id = 0;
    // 记录开始时间
...
int main(int argc, char **argv)
{
    ...
    // 参数：是否录像
    const bool record = (argc > 4) ? atoi(argv[4]) : false;
    ...
    // 启动结果线程
    std::thread result_thread(get_results, 854, 480, 25, record);
