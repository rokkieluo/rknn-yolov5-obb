...
public:
    ...
    nn_error_e Run(const cv::Mat &img, std::vector<ObbDetection> &objects); // 运行模型

private:
    nn_error_e Preprocess(const cv::Mat &img, const std::string process_type,cv::Mat &image_letterbox);   // 图像预处理
    nn_error_e Inference();                                                      // 推理
    nn_error_e Postprocess(const cv::Mat &image_letterbox, std::vector<ObbDetection> &objects); // 后处理

    LetterBoxInfo letterbox_info_;
    int original_img_width_;  // 原始图像宽度
    int original_img_height_; // 原始图像高度
